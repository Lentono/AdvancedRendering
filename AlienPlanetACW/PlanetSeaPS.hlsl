...
float4 PhongIllumination(float3 pos, float3 normal, float3 viewDir, float4 diffuse, float2 texcoord)
{
	...
	for (int i = 0; i < NUMBER_OF_LIGHTS; i++)
	{
		...
		if (nDotL > 0.0f)
		{
			float4 specularIntensity = specularTexture.Sample(sampleType, texcoord);
...
float4 main(PixelShaderInput input) : SV_TARGET
{
	...
	{
		float4 colour = PhongIllumination(input.positionW, bumpNormal, input.viewDirection, saturate(float4(1.0f, 1.0f, 1.0f, 1.0f)), input.tex);

		return float4(lerp(colour.xyz, float3(1.0f, 0.97255f, 0.86275f), exp(-5.0*t)), 1.0f);
	}*/

	float4 colour = PhongIllumination(input.positionW, bumpNormal, input.viewDirection, saturate(float4(0.0f, 1.0f, 0.67f, 1.0f)), input.tex);
