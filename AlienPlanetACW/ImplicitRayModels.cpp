...
ImplicitRayModels::ImplicitRayModels(const std::shared_ptr<DX::DeviceResources>& deviceResources) : m_deviceResources(deviceResources), m_loadingComplete(false), m_indexCount(0)
{
	CreateDeviceDependentResources();
}

void ImplicitRayModels::CreateDeviceDependentResources()
{
	// Load shaders asynchronously.
	auto loadVSTask = DX::ReadDataAsync(L"ImplicitRayModelsVS.cso");
	auto loadPSTask = DX::ReadDataAsync(L"ImplicitRayModelsPS.cso");
...
void ImplicitRayModels::SetViewProjectionMatrixConstantBuffer(DirectX::XMMATRIX& view, DirectX::XMMATRIX& projection)
{
...
void ImplicitRayModels::SetInverseViewMatrixConstantBuffer(DirectX::XMMATRIX& inverseView)
{
	DirectX::XMStoreFloat4x4(&m_inverseViewBufferData.inverseView, DirectX::XMMatrixTranspose(inverseView));
}

void ImplicitRayModels::SetCameraPositionConstantBuffer(DirectX::XMFLOAT3& cameraPosition)
{
	m_cameraBufferData.position = cameraPosition;
}

void ImplicitRayModels::ReleaseDeviceDependentResources()
{
...
void ImplicitRayModels::Update(DX::StepTimer const& timer)
{
