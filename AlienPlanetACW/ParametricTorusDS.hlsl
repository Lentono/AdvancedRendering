...
PixelShaderInput main(in PatchConstantOutput input, in const float2 uvCoord : SV_DomainLocation, const OutputPatch<DomainShaderInput, 4> patch)
{
	...
	output.positionW.x = (1.0f + 0.5f * cos((uvCoord.y + 0.5f) * (2 * PI))) * cos((uvCoord.x + 0.5f) * (2 * PI));
	output.positionW.y = (1.0f + 0.5f * cos((uvCoord.y + 0.5f) * (2 * PI))) * sin((uvCoord.x + 0.5f) * (2 * PI));
	output.positionW.z = 0.5f * sin((uvCoord.y + 0.5f) * (2 * PI));
